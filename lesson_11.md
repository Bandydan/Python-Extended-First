...
`truncate table` с именем таблицы удаляет только содержимое таблицы, не меняя ее структуры, но не пересоздавая первичный ключ, так что он продолжаться далее. Для того чтоб новые данные добавлять "с единички" `truncate [table name] restart identity;`.

База данных удаляется командой `drop database` с именем базы данных и точкой запятой в конце. 
